...
base_dir, summit_file, stage_tag = sys.argv[1], sys.argv[2], sys.argv[3]
gold_file = os.path.join(base_dir,
                         f"competition_stage_{stage_tag}_test_truth.txt")  # competition_stage_2_test_truth.txt competition_stage_3_test_truth.txt
...
for i in range(len(gold_data)):
    ...
    """去重"""
    selected_id_a = list(set(selected_id_a))
    selected_id_b = list(set(selected_id_b))
    pre_relation_str = []
    for item in pre_relation:
        pre_relation_str.append(str(item))

    pre_relation = []
    for item in list(set(pre_relation_str)):
        pre_relation.append(eval(item))


    relations_score = rationale_acc(selected_id_a, Case_a_rational, selected_id_b, Case_b_rational, pre_relation,
                                    relations)
